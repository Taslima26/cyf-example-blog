...
server.get('/posts.json',  (req, res) => {
  const {titleQParam, bodyQParam, limit} = req.query;
  let foundPosts = database.posts;
  if (titleQParam !== undefined) {
    foundPosts = database.posts.filter(post => element.title.startsWith(titleQParam)) 
  }
  if (bodyQParam !== undefined) {
    foundPosts = database.posts.filter(post => element.title.includes(bodyQParam))
  }
  if (limit !== undefined) {
